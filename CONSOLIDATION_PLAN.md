...
>
> `pr-description-add` and `test-and-analyse` run ncc `dist/`. That bundle is committed only on suite release. `@main` for those two actions does not run current source.
