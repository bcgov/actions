...
export function parseJUnitXmlContent(xmlContent) {
  ...
  rawSuites.forEach(suite => {
    ...
    results.failed +=
      parseInt(suite.failures || 0) + parseInt(suite.errors || 0)
...
export function summarizeJUnitXmlFiles(xmlFiles) {
  ...
  testResults.passed =
    testResults.total - testResults.failed - testResults.skipped
  return testResults
}
...
async function run() {
  try {
    ...
    if (process.env.GITHUB_STEP_SUMMARY) {
      ...
      if (language === 'node') {
        ...
        if (fs.existsSync(knipSummaryFile)) {
          ...
          summary
            .addHeading('📋 Knip Full Output', 3)
            .addCodeBlock(detail.replace(ansiRegex, ''))
        }
      }
    ...
    if (hasTestFailures) {
      failures.push(
        `${testResults.failed} tests failed in the ${language} project.`
      )
    }
